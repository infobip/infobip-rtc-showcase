...
class AppController: UIViewController {
    ...
    var audioFilter: AudioSource? = nil

    override func viewDidLoad() {
        super.viewDidLoad()
        self.hideKeyboardWhenTappedAround()
        self.setStatusBarStyle()
        self.initializeAudioSource()
    ...
    func initializeAudioSource() {
        // When launching the app, we need to tell the SDK to not use the built in ADM (which uses device hardware),
        // but supply our custom impl It's important to know what sample rate your input has, otherwise it will
        // crash due to exceptions when processing the audio internally.
        AudioDeviceModule.instance = AudioDeviceModuleFactory.createNativeWithExternalInput(
                channels: 1, sampleRateHz: 44100
        )

        let url = Bundle.main.url(forResource: "CLASSICA", withExtension: "wav")!
        do {
            let buffer = try AVAudioPCMBuffer.init(url: url)
            self.audioFilter = AudioSourcePcmData(buffer!)
        } catch {
            print("Failed to load wav file!")
        }

        self.audioFilter!.start()
    }
